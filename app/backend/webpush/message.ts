...
import type { WebPushInfos } from './webpushinfos.ts';
import { b64ToUrlEncoded, cryptoKeysToUint8Array, exportPublicKeyPair, joinUint8Arrays, stringToU8Array, u8ToString } from './util.ts';
import { hkdfGenerate } from './hkdf.ts';
...
function generateSalt(length = 16): Promise<Uint8Array> {
    return Promise.resolve(crypto.getRandomValues(new Uint8Array(length)))
}
